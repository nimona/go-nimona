...
func main() {
	...
	bootstrapPeerInfos = append(bootstrapPeerInfos, p1.Message())
	...
	for size := 1000; size <= 500000; size = size + 1000 {
		...
		start := time.Now()
		for i := 0; i < count; i++ {
			err := w2.Send(ctx, "foo", "bar", payload, []string{p1ID})
			if err != nil {
				fmt.Println("could not send message, error:", err)
			}
		}
		elapsed := time.Since(start)
		fmt.Printf("%d, %d\n", size/1000, elapsed.Nanoseconds()/int64(count)/int64(time.Millisecond))
	}
...
func newPeer(port int) (*net.SecretPeerInfo, net.Messenger, net.AddressBook) {
	...
	for _, peerInfo := range bootstrapPeerInfos {
		reg.PutPeerInfoFromMessage(&peerInfo)
	}
