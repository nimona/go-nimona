    ...
    units:
      - name: nimona.service
        ...
        content: |
          ...
          ExecStartPre=/usr/bin/docker pull nimona/nimona:{{ .docker.tag}}
          ExecStart=/usr/bin/docker run --name nimona --rm -p 21013:21013  -p 8080:8080 nimona/nimona daemon --port=21013 --api-port=8080 --announce-hostname={{ .hostname }}
