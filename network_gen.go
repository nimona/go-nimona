...
func (t *NetworkAlias) MarshalCBOR(w io.Writer) error {
	...
	if err := cw.WriteMajorTypeHeader(cbg.MajTextString, uint64(len("core/network/alias"))); err != nil {
		return err
	}
	if _, err := io.WriteString(w, string("core/network/alias")); err != nil {
		return err
	}
...
func (t *NetworkAlias) UnmarshalCBOR(r io.Reader) (err error) {
	...
	if extra > cbg.MaxLength {
		return fmt.Errorf("NetworkAlias: map struct too large (%d)", extra)
	}
...
func (t *NetworkIdentity) MarshalCBOR(w io.Writer) error {
	...
	// t.NetworkInfoRootID (nimona.DocumentID) (struct)
	if !zero.IsZeroVal(t.NetworkInfoRootID) {

		if len("networkInfoRootID") > cbg.MaxLength {
			return xerrors.Errorf("Value in field \"networkInfoRootID\" was too long")
		}

		if err := cw.WriteMajorTypeHeader(cbg.MajTextString, uint64(len("networkInfoRootID"))); err != nil {
			return err
		}
		if _, err := io.WriteString(w, string("networkInfoRootID")); err != nil {
			return err
		}
...
func (t *NetworkIdentity) UnmarshalCBOR(r io.Reader) (err error) {
	...
	if extra > cbg.MaxLength {
		return fmt.Errorf("NetworkIdentity: map struct too large (%d)", extra)
	}
