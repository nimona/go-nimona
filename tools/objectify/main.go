...
func (s *{{ .StructName }}) FromObject(o *object.Object) error {
	{{- range .StructFields }}
		...
		{{- else if eq .Type "&object.Object" }}
			if v, ok := o.GetRaw("{{ .Tag }}").(*object.Object); ok {
				s.{{ .Name }} = v
			}
		{{- else if .IsSlice }}
			if ss, ok := o.GetRaw("{{ .Tag }}").([]{{ .ElemType }}); ok {
				...
				for _, si := range ss {
					...
					{{- if not .IsBasic }} else if v, ok := si.(*object.Object); ok {
						s{{ .Name }} := {{ .ElemTypePtr }}{}
						if err := s{{ .Name }}.FromObject(v); err != nil {
							return err
						}
		...
		if v, ok := o.GetRaw("{{ .Tag }}").({{ .TypePtr }}); ok {
			s.{{ .Name }} = v
		} else if v, ok := o.GetRaw("{{ .Tag }}").(*object.Object); ok {
			s.{{ .Name }} = {{ .Type }}{}
			s.{{ .Name }}.FromObject(v)
		}
		{{- else }}
			if v, ok := o.GetRaw("{{ .Tag }}").({{ .Type }}); ok {
				s.{{ .Name }} = v
			}
		{{- end }}
	{{- end }}
