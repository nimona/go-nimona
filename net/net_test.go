...
func TestNetResolver(t *testing.T) {
	...
	p2, err := n1.Resolver().Resolve(n2.key.HashBase58())
	assert.NoError(t, err)
	assert.Equal(t, n2.key.GetPublicKey(), p2.SignerKey.GetPublicKey())
	assert.Equal(t, n2.key.GetPublicKey().HashBase58(), p2.SignerKey.GetPublicKey().HashBase58())

	p1, err := n2.Resolver().Resolve(n1.key.HashBase58())
	assert.NoError(t, err)
	assert.Equal(t, n1.key.GetPublicKey(), p1.SignerKey.GetPublicKey())
	assert.Equal(t, n1.key.GetPublicKey().HashBase58(), p1.SignerKey.GetPublicKey().HashBase58())
}
